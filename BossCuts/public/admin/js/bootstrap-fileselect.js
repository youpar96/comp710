(function (window, $) {
    ...
    var Fileselect = function (fileInput, options) {
        ...
        this.$inputGroup.css("width", "50%");
        this.$inputGroup.css("float", "left");
        this.$inputGroupBtn = $('<label>').addClass('input-group-btn');
        this.$browseBtn = $('<span>');
        this.$labelInput = $('<input>').attr('type', 'text').attr('readonly', true).addClass('form-control').attr("required", "required");
        this.translations = {
            'en': {
                'browse': 'Browse',
                'rules': {
